...
def bwas_cpp(args, env: Environment, states: List[State], results_file: str):
    # UPDATED TO INCLUDE CUBE222
    assert (args.env.upper() in ['CUBE222', 'CUBE3', 'CUBE4', 'PUZZLE15', 'PUZZLE24', 'PUZZLE35', 'PUZZLE48', 'LIGHTSOUT7'])
    ...
    for state_idx, state in enumerate(states):
        ...
        if args.env.upper() == "CUBE3":
            ...
            state_str: str = " ".join([str(x) for x in state.cube])
        else:
            raise ValueError("Unknown c++ environment: %s" % args.env)
