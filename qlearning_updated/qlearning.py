...
def train(q_table, env_name, episodes, max_moves = 1000, lr = 0.1, discount = 1, epsilon = 1):
    ...
    for episode in range(episodes):
        ...
        if env_name == "2x2x2":
            state, _ = env.reset_scramble(num_scramble = random.randint(1,14))
        elif env_name == "skewb":
            state, _ = env.reset(num_scramble= random.randint(1,14))

        episode_reward, success = 0, 0
        done, count = False, 0

        while not done and count <= max_moves: # run for at most max moves steps in the environment -> ensure termination
...
def plot(env_name, GROUP_SIZE, episode_rewards, episode_success):
    ...
    with open(env_name + "_random_scramble"+op + "_stats" + ".pickle", "wb") as f:
    ...
    fig.savefig(env_name + "_random_scramble"+op + ".png", format='png', dpi=100, bbox_inches='tight')
...
def save_qtable(env_name, q_table):
    with open(env_name + "_random_scramble"+op + "_qtable" + ".pickle", "wb") as f:
...
if __name__ == '__main__':
    ...
    op = f"{env_name}_random_scramble_{episodes}_{max_moves}"
