...
if __name__ == '__main__':
    ...
    for i in range(1400):
        if i%100 == 0:
            episode_rewards_dict[int(i/100)+1] = []
            episode_success_dict[int(i/100)+1] = []
            episode_num_moves_dict[int(i/100)+1] = []
        done = True
        while done:
            state, _ = env.reset_scramble(num_scramble=int(i/100)+1)
            #state, _ = env.reset(num_scramble=int(i/100)+1) # for skewb
            _, done = env.reward()

        reward, success, num_moves = eval_scramble(state)
        episode_rewards_dict[int(i/100)+1].append(reward)
        episode_success_dict[int(i/100)+1].append(success)
        if success == 100:
            episode_num_moves_dict[int(i/100)+1].append(num_moves)
        if i%100 == 99:
